...
def _fact(spark, rows=DATASET, snapshot="2026-01-01"):
    raw = spark.createDataFrame([dict(zip(COLUMNS, r)) for r in rows])
    return s2g.build_fact(b2s.build_silver(raw, snapshot))


def test_build_fact_dedups_cross_source(spark):
    # 6 raw rows, but the DevOps posting on two boards collapses -> 5 unique content rows.
    assert _fact(spark).count() == 5


def test_demand_by_role_counts(spark):
    roles = s2g.make_roles_df(spark)
    demand = {r["role"]: r["job_count"] for r in s2g.build_demand_by_role(_fact(spark), roles, "2026-01-01").collect()}
...
def test_role_opportunity_decision_columns(spark):
    roles = s2g.make_roles_df(spark)
    opp = {r["role"]: r for r in s2g.build_role_opportunity(_fact(spark), roles, "2026-01-01").collect()}
