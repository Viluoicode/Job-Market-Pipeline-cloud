...
def test_dedup_within_source_keeps_latest(bronze):
    ...
    out = b2s.build_silver(bronze(rows), "2026-02-01").collect()
    assert len(out) == 1
    assert out[0]["description"] == "new"          # latest posted_at wins
    assert out[0]["job_id"] == out[0]["job_id"].lower()


def test_dedup_hash_equal_across_sources_for_same_content(bronze):
    ...
    hashes = {r["dedup_hash"] for r in b2s.build_silver(bronze(rows), "2026-01-01").collect()}
    assert len(hashes) == 1


def test_job_id_differs_across_sources(bronze):
    ...
    ids = {r["job_id"] for r in b2s.build_silver(bronze(rows), "2026-01-01").collect()}
    assert len(ids) == 2


def test_drops_rows_without_source_job_id(bronze):
    rows = [
        ("greenhouse", "a", "", "Acme", "X", "NYC", False, "u", "2026-01-01T00:00:00+00:00", "x"),
        ("greenhouse", "a", "2", "Acme", "X", "NYC", False, "u", "2026-01-01T00:00:00+00:00", "y"),
    ]
    assert b2s.build_silver(bronze(rows), "2026-01-01").count() == 1


def test_remote_defaults_to_false_when_null(bronze):
    # A board that omits the remote flag must land as False, never NULL.
    rows = [
        ("greenhouse", "a", "1", "Acme", "X", "NYC", None, "u", "2026-01-01T00:00:00+00:00", "x"),
    ]
    assert b2s.build_silver(bronze(rows), "2026-01-01").collect()[0]["is_remote"] is False
